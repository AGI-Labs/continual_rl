...
class EWCPolicyConfig(ImpalaPolicyConfig):
    ...
    def __init__(self):
        ...
        self.task_replay_buffer_max_size = int(1e6) # save a buffer per task for computing Fisher estimates

        self.n_fisher_samples = 100 # num of batches to draw to recompute the diagonal of the Fisher
        self.ewc_lambda = 400 # "tuned choosing from [500, 1000, 1500, 2000, 2500, 3000]? exact value not specified by Progerss & Compress"
        ...
        # self.ewc_lambda = None # "As the scale of the losses differ, we selected λ for online EWC as applied in P&C among [25, 75, 125, 175]."
        # self.online_gamma = 0.99
        self.online_gamma = 0.95 # "γ < 1 is a hyperparameter associated with removing the approximation term associated with the previous presen-tation of task i."
        self.normalize_fisher = False # "We counteract this issue by normalising the Fisher information matrices Fi for each task.""
